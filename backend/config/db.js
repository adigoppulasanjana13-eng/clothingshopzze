...
const connectDB = async () => {
  try {
    ...
    if (process.env.VERCEL) {
      console.error('Running on Vercel: In-memory MongoDB Server cannot be started. Please configure the MONGO_URI environment variable in Vercel settings.');
      throw error;
    }
    console.log('Spinning up MongoDB Memory Server (in-memory mode)...');
    try {
      ...
      // Programmatically check and seed data
      await checkAndSeedData();
    } catch (memError) {
      console.error(`In-memory database startup failed: ${memError.message}`);
      process.exit(1);
    }
  }
};
...
const checkAndSeedData = async () => {
  try {
    ...
    console.log('Database empty. Performing programmatic self-seeding of products, users, and coupons...');
    ...
    const products = [
      ...
      },
      {
        ...
        description: 'Authentic rigid cotton denim in a classic boyfriend silhouette, featuring heavily distressed knees and fraying hems for a lived-in feel.',
        category: 'Bottoms',
        ageGroup: 'Young Women',
        ageMin: 18,
        ageMax: 25,
    ...
    // Seed reviews on featured items to make it look premium
    const reviewer = await User.findOne({ role: 'user' });
    const reviewerId = reviewer ? reviewer._id : new mongoose.Types.ObjectId();
    const reviewerName = reviewer ? reviewer.name : 'Sathwika D';
